...
def step(planner:HumanoidMotionPlanner):
    ...
    constraints.append(
        Constraint(
            ...
            object_key_point=get_point_in_env(planner.env,type_name="can",obj_id=0)+np.array([-0.1,0.2,0.2]),
            # object_key_point=get_point_in_env(planner.env,type_name="can",obj_id=0)+np.array([-0.1,0.2,0.1]),
        )
    )
    constraints.append(
        Constraint(
