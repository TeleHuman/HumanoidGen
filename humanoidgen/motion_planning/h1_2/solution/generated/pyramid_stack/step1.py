...
def step(planner:HumanoidMotionPlanner):
    ...
    constraint_l=planner.generate_constraints(obj_name="cube", obj_id=0, action="pinch", hand_name="left")
    _, target_effector_pose_l = planner.generate_end_effector_pose(constraint_l,hand_name="left")
    constraint_r=planner.generate_constraints(obj_name="cube", obj_id=1, action="pinch", hand_name="right")
